    ...
    public function index(Request $request) 
    {
        ...
        if ($request->ajax()) {
            ...
            ->addColumn('action', function ($user) {
                    $buttons = '';
                    {
                        $buttons .= '<a href="' . route('users.edit', $user->id) . '" class="btn btn-icon btn-sm btn-warning me-1" title="Ubah">
                                        <i class="ti ti-edit"></i>
                                    </a>';
                    }
                    {
                        $buttons .= '<a href="' . route('users.show', $user->id) . '" class="btn btn-icon btn-sm btn-primary me-1" title="Lihat">
                                        <i class="ti ti-eye"></i>
                                    </a>';
                    }
                    {
                        $buttons .= '<button data-id="' . $user->id . '" class="btn btn-icon btn-sm btn-danger delete-user" title="Hapus">
