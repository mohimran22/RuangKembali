...
class TeamController extends Controller
{
    ...
    public function create()
    {
        ...
        $roles = Role::internal()
            ->orderBy('name')
            ->get();
        return view('teams.create', compact('user', 'roles', 'religions', 'provinces'));
    }
...
public static function generateNikAjax()
{
    ...
    public function edit($id)
    {
        ...
        $user = $team->user()->with('bank')->first();
        $religions = Religion::all();
        $provinces = Province::all();
        $roles = Role::all();
    ...
    public function update(Request $request, Team $team)
{
    $validated = $request->validate([
        ...
        'identity_photo' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:5120',
        'nik' => 'required|string|max:50|unique:teams,nik,' . $team->id,
        'role' => 'required|array',
        'role.*' => 'string|exists:roles,name',
        'marital_status' => 'nullable|in:1,2,3,4',
    ...
    if ($request->hasFile('contract_letter_file')) {
        if ($team->contract_letter_file) {
            Storage::disk('public')->delete($team->contract_letter_file);
        }
    ...
    if ($request->hasFile('training_certificate')) {
        if ($team->training_certificate) {
            Storage::disk('public')->delete($team->training_certificate);
        }
    ...
    $team->update([
        ...
        'bonus' => $validated['bonus'],
        'thr' => $validated['thr'],
        'contract_letter_file' => $validated['contract_letter_file'] ?? $team->contract_letter_file,
        'training_certificate' => $validated['training_certificate'] ?? $team->training_certificate,
    ]);
    });
